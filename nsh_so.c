...
const char *shm_base_file = "shm_base"; //共享内存文件
 ...
 * 获取新的描述符
 */
int socket(int af, int socket, int type) {
 ...
 * 确定最大连接数，初始化相关内容
 */
int listen(int fd, int max_connect) {
    //根据描述符读取对象，判断是否有效
    shm_socket shm_socket_obj = get_socket(fd);
    if (shm_socket_obj->valid == 0) {
        return -1;
...
int connect(int fd, struct sockaddr *addr, socklen_t addr_len) {
    //初始化描述符
    shm_socket_init(fd);
    shm_socket shm_socket_obj = get_socket(fd);
 ...
 * 接收消息，阻塞
 */
int recv(int fd, void *buf, size_t buf_size, int type) {
    //根据描述符读取对象，判断是否有效
    shm_socket shm_socket_obj = get_socket(fd);
    if (shm_socket_obj->valid == 0) {
        return -1;
 ...
 * 发送消息（阻塞）
 */
int send(int fd, void *data, size_t size, int type) {
    //根据描述符读取对象，判断是否有效
    shm_socket shm_socket_obj = get_socket(fd);
    if (shm_socket_obj->valid == 0) {
        return -1;
